...
ADDR=$(hostname)
SPITZUID="$ADDR"-"$$"
OUTPUTFILE="$LOGSDIR/jobmanager-$SPITZUID.stdout"
LOGFILE="$LOGSDIR/jobmanager-$SPITZUID.stderr"
...
show_script_params() {
    ...
    echo -e "\t\e[1mUsing '$SPITZUID' as UID\e[21m"
...
run_jobmanager() {
    ...
    SPITSBIN+="/media/napoli/d9b9ded0-5c92-4552-9238-9aaba7ffb8e6/Documents/Douturado/spitz/bin/spitz-pi/spitz-pi-module"
    SPITSARGS+="100000"

    JM_CMD="$PYTHONCMD $SPITSDIR/$JOBMANAGER $JOBMANAGER_ARGS $SPITSBIN $SPITSARGS"
    echo $SPITZUID > $JOBDIR/jm.exists
    # Wait to NFS sync...
    sleep 1

    if [ "x$SPITZUID" = "x$(cat $JOBDIR/jm.exists)" ]
    then
